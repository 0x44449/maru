...
email            = JWT email claim (사용자 식별 기준, UNIQUE)
...
**인증**: Bearer JWT 필요, X-Profile-Id 불필요 (email 기준으로 사용자 식별, PERSONAL 프로필만 수정)
...
| `UserEntity.java` | 수정 | user_id → UUID, user_tag 추가, auth_provider 관련 컬럼 제거 |
| `ProfileEntity.java` | 수정 | profile_id/user_id → UUID |
| 기타 엔티티 | 수정 | VARCHAR ID → UUID 타입 변경 |
| `CredentialPayload.java` | 수정 | profileId 필드 추가, auth_provider 관련 필드 제거 |
| `RequestPayloadResolver.java` | 수정 | email 기반 식별 + X-Profile-Id 파싱 |
| `UserController.java` | 수정 | provision, getMe, updateMe, checkId |
| `UserService.java` | 수정 | 프로비저닝 로직, 조회 로직, 프로필 설정 로직 |
| `UserRepository.java` | 수정 | findByEmail 기반 조회, auth_provider 복합 조회 제거 |
...
1. **email**: JWT에서 추출. 클라이언트가 보내지 않음. 사용자 식별 기준.
...
8. **email 기반 식별**: 소셜 프로바이더에 관계없이 동일 이메일이면 동일 사용자로 판단.
